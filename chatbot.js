(function() {
  ...
  // Define Mock Consignment Tracking Database (synced with index.js)
  const trackingData = {
    'GNX-10294': {
      origin: 'Mumbai (MH)',
      dest: 'Gurugram (HR)',
  ...
  function appendQuickReplies() {
    ...
    const options = [
      { text: 'Track Shipment 📦', value: 'track' },
  ...
  function getResponseText(text) {
    ...
    // Check for specific consignment tracking number formats (e.g. GNX-10294, GNX-88402, GNX-12345)
    const gnxMatch = query.match(/gnx\s*-?\s*(\d+)/i);
    if (gnxMatch || query.includes('track') || query.includes('consignment') || query.includes('shipment')) {
      let trackingId = '';
      if (gnxMatch) {
        trackingId = 'GNX-' + gnxMatch[1];
      } else {
        // Extract plain numbers if available
        const numMatch = query.match(/\d+/);
        if (numMatch) {
          trackingId = 'GNX-' + numMatch[0];
        }
      }

      if (trackingId) {
        const data = trackingData[trackingId] || trackingData['GNX-DEFAULT'];
        const displayId = trackingData[trackingId] ? trackingId : 'GNX-MOCK-DISPATCH';
        return `📦 <strong>Consignment Found: ${displayId}</strong><br><br>` +
               `• <strong>Origin ➔ Destination:</strong> ${data.origin} to ${data.dest}<br>` +
               `• <strong>Current Status:</strong> <span style="color: var(--chat-accent-green); font-weight: bold;">${data.status}</span><br>` +
               `• <strong>Est. Delivery:</strong> ${data.estArrival}<br>` +
               `• <strong>Journey Progress:</strong> ${data.progress}%<br><br>` +
               `You can also use the Consignment Tracker widget on our homepage to view the progress bar animations.`;
      }
      
      return `📋 To track a cargo shipment, please reply with your **Consignment ID** (for example, try: **GNX-10294** or **GNX-88402**), or enter it directly in the tracker input widget on our homepage.`;
    }
  ...
  window.addEventListener('hashchange', () => {
    if (window.location.hash === '#tracker-section') {
      // Prompt user in chat if window is active
      if (chatbotWindow.classList.contains('active')) {
        setTimeout(() => {
          appendMessage("💡 You navigated to our Consignment tracker! Enter your ID here (e.g. GNX-10294) to trace the transit progress immediately.", 'bot');
        }, 600);
      }
    }
  });
